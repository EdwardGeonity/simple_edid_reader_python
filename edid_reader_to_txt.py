...
def safe_ascii(b: bytes) -> str:
    # EDID text fields: ASCII, often padded with spaces and ends with 0x0A
    s = b.decode("ascii", errors="ignore")
    s = s.replace("\x00", "")
    return s.strip(" \r\n\t")

def checksum_ok(block128: bytes) -> bool:
    # Valid when sum of all 128 bytes == 0 mod 256
    return (sum(block128) & 0xFF) == 0

def decode_manufacturer_id(word_be: bytes) -> str:
    # 2 bytes big-endian, 3 letters packed 5-bit each
...
# ============================================================
# Byte labels for EDID 1.4 Base Block
# ============================================================
...
def decode_video_input_definition(v: int) -> List[str]:
    out = [f"Raw: 0x{v:02X} (bits {bits8(v)})"]
    if v & 0x80:
        ...
        out.append(f"Color bit depth (bits6..4): {depth} -> {depth_map.get(depth, 'Unknown')}")
        iface = v & 0x0F
        iface_map = {
            0x0: "Undefined",
            0x1: "DVI",
...
def decode_feature_support(f: int) -> List[str]:
    ...
    out.append(f"Color Type (bits4..3): {color_type} -> {color_map.get(color_type, 'Unknown')}")
...
def decode_established_timings(b0: int, b1: int, b2: int) -> List[str]:
    out = [f"Raw bytes: {b0:02X} {b1:02X} {b2:02X}"]
...
def decode_dtd(d: bytes) -> List[str]:
    ...
    # EDID DTD flags interpretation (safe)
    ...
    sync_map = {
        ...
        1: "Bipolar analog composite sync",
        2: "Digital composite sync",
        3: "Digital separate sync",
    }

    out.append(f"Interlaced (bit7): {'Yes' if interlaced else 'No'}")
    out.append(f"Stereo mode (bits6..5): {stereo} -> {stereo_map.get(stereo, 'Unknown')}")
    out.append(f"Sync type (bits4..3): {sync_type} -> {sync_map.get(sync_type, 'Unknown')}")

    # For digital separate sync (sync_type == 3), bits2..1 are polarities.
    if sync_type == 3:
        ...
        out.append("bit0: Reserved/Don't care for separate sync")
    else:
        out.append("Note: lower flag bits interpretation depends on sync type; shown raw only.")
...
def decode_monitor_descriptor(d: bytes) -> List[str]:
    ...
    # EDID: 0x00..0x0F are Manufacturer Specified Display Descriptors (valid but vendor-defined)
    if 0x00 <= tag <= 0x0F:
        out.append("Type: Manufacturer-Specified Display Descriptor (tag 0x00..0x0F)")
    ...
    if tag == 0xFC:
        ...
        out.append(f"  Additional bytes: {hx(d[10:18])}")
    else:
        out.append("Type: Standard/Reserved monitor descriptor (not fully decoded here).")
        out.append(f"Raw payload: {hx(payload)}")

    return out

def decode_descriptor(d: bytes, index: int) -> List[str]:
    out = [f"Descriptor #{index} raw: {hx(d)}"]
...
# ============================================================
# Report builder (write to TXT)
# ============================================================

def build_report(edid128: bytes) -> str:
    edid = edid128[:128]
    ...
    lines.append("="*78)
    lines.append("EDID Base Block (128 bytes) — FULL DETAILED PARSE (English)")
    lines.append("="*78)
    lines.append("")
    ...
    lines.append(f"Header: {hx(header)}")
    lines.append(f"Manufacturer ID: {hx(edid[8:10])} -> {man}")
    lines.append(f"Product Code: 0x{prod:04X} ({prod})")
    lines.append(f"Serial Number: 0x{serial:08X} ({serial})")
    lines.append(f"Manufacture Date: week={week}, year={year}")
    lines.append(f"EDID Version: {ver[0]}.{ver[1]}")
    lines.append(f"Extension Block Count @0x7E: {ext_count}")
    lines.append(f"Checksum @0x7F: 0x{cs:02X} | Valid: {'YES' if ok else 'NO'}")
    lines.append("")

    # Byte map
    lines.append("-"*78)
    lines.append("[Byte Map: offset -> hex -> bits -> meaning]")
    lines.append("-"*78)
    for off in range(128):
        b = edid[off]
        label = labels.get(off, "Reserved/Unknown")
        lines.append(f"0x{off:02X}  {b:02X}  {bits8(b)}   {label}")
    lines.append("")

    # Video Input
    lines.append("-"*78)
    lines.append("[Video Input Definition @0x14]")
    lines.append("-"*78)
    lines.extend(decode_video_input_definition(edid[0x14]))
    lines.append("")

    # Basic Display Parameters
    lines.append("-"*78)
    lines.append("[Basic Display Parameters @0x15..0x18]")
    lines.append("-"*78)
    lines.append(f"Max Image Size: {edid[0x15]} x {edid[0x16]} cm (0 means 'unspecified')")
    lines.append(decode_gamma(edid[0x17]))
    lines.append("")
    lines.append("Feature Support @0x18:")
    for s in decode_feature_support(edid[0x18]):
        lines.append("  " + s)
    lines.append("")

    # Chromaticity
    lines.append("-"*78)
    lines.append("[Chromaticity @0x19..0x22]")
    lines.append("-"*78)
    lines.extend(decode_chromaticity(edid))
    lines.append("")

    # Established Timings
    lines.append("-"*78)
    lines.append("[Established Timings @0x23..0x25]")
    lines.append("-"*78)
    lines.extend(decode_established_timings(edid[0x23], edid[0x24], edid[0x25]))
    lines.append("")

    # Standard timings
    lines.append("-"*78)
    lines.append("[Standard Timings @0x26..0x35 (8 entries)]")
    lines.append("-"*78)
    for i in range(8):
        b1 = edid[0x26 + i*2]
        b2 = edid[0x27 + i*2]
        lines.append(f"#{i+1}: {decode_standard_timing(b1, b2)}")
    lines.append("")

    # Descriptors
    lines.append("-"*78)
    lines.append("[Descriptors @0x36..0x7D (4 x 18 bytes)]")
    lines.append("-"*78)
    for i in range(4):
        base = 0x36 + i*18
        d = edid[base:base+18]
        for s in decode_descriptor(d, i+1):
            lines.append(s)
    lines.append("")

    # Trailer
    lines.append("-"*78)
    lines.append("[Trailer @0x7E..0x7F]")
    lines.append("-"*78)
    lines.append(f"Extension Block Count (0x7E): {edid[0x7E]}")
    total = sum(edid) & 0xFF
    lines.append(f"Checksum byte (0x7F): 0x{edid[0x7F]:02X}")
    lines.append(f"Sum(all 128 bytes) & 0xFF: {total:02X} -> {'VALID' if total == 0 else 'INVALID'}")
    lines.append("")
    lines.append("="*78)
    lines.append("Done.")
    lines.append("="*78)
...
def choose_input_path() -> str:
    # Ask user for filename if not provided
    inp = input("Enter EDID binary filename (e.g. edid.bin): ").strip().strip('"').strip("'")
    return inp

def make_output_txt_path(input_path: str) -> str:
    # Save next to input file, same base name but .txt
...
def main():
    ...
    if not os.path.exists(input_path):
        # also try relative to script folder
        script_dir = os.path.dirname(os.path.abspath(__file__))
        alt = os.path.join(script_dir, input_path)
    ...
    if len(data) < 128:
        print(f"ERROR: file too small ({len(data)} bytes). Need at least 128 bytes.")
        sys.exit(1)

    edid128 = data[:128]

    # 3) Build report text
    report = build_report(edid128)

    # 4) Write report to TXT
    out_path = make_output_txt_path(input_path)
    with open(out_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(report)

    # 5) Minimal console output (not the report)
    print(f"OK: Parsed 128-byte EDID base block from: {input_path}")
